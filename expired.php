    ...
    <div class="bg-gray-50 min-h-screen hidden md:block">
        <div class="grid grid-cols-6">
            ...
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-gray-400 font-semibold text-2xl mb-3">Expired Products</h2>
                ...
                <?php
                    ...
                    if(!$conn) {
                        ...
                        $sql = "select * from products where expiryDate < '$current_date'";
                ...
                <div class="rounded-md mb-4">
                    ...
                    <div class="flex items-center justify-center">
                        <div class="flex justify-between text-lg">
                            <span class="fa fa-eye text-blue-600 hover:text-blue-800 cursor-pointer" onclick="viewProduct(<?php echo $row['id']; ?>)"></span>
                            <span class="fa fa-check ml-4 text-blue-600 hover:text-blue-800 cursor-pointer" onclick="checkProduct()"> </span> <span class="fa fa-trash ml-4 text-red-600 hover:text-red-800 cursor-pointer" onclick="deleteProduct()"> </span>
                        </div>
                    </div>
                </div>
                </div>
                    ...
                    <div class="bg-white shadow-lg w-2/7 z-50 rounded-lg">
                        <div class="p-6">
                            ...
                            <div class="flex justify-center">
                                <button class="py-2 px-4 bg-red-400 hover:bg-red-500 rounded-md text-white transition ease-in duration-300 w-1/3 mr-3" onclick="deleteP(<?php echo $row['id']; ?>)">Yes</button>
                                <button class="py-2 px-4 bg-green-400 hover:bg-green-500 rounded-md text-white transition ease-in duration-300 w-1/3" onclick="deleteProduct()">No</button>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="h-screen w-full fixed left-0 top-0 flex justify-center items-center hidden" id="checkButton" style="zIndex: 1000">
                    <div class="h-screen w-full absolute z-50 bg-black bg-opacity-50 cursor-pointer" onclick="checkProduct()"></div>
                    <div class="bg-white shadow-lg w-2/7 z-50 rounded-lg">
                        <div class="p-6">
                            <h4 class="tracking-md text-xl text-center mb-3 font-semibold">Are you sure you want to remove this expired product?</h4>
                            <div class="flex justify-center">
                                <button class="py-2 px-4 bg-red-400 hover:bg-red-500 rounded-md text-white transition ease-in duration-300 w-1/3 mr-3" onclick="deleteP(<?php echo $row['id']; ?>)">Yes</button>
                                <button class="py-2 px-4 bg-green-400 hover:bg-green-500 rounded-md text-white transition ease-in duration-300 w-1/3" onclick="deleteProduct()">No</button>
                            </div>
                ...
                <?php
                            ...
                            echo "<h2 class='font-bold text-lg text-gray-600'>There are no expired products</h2>";
                        }
                        } else {
...
// Functions to view or hide modals
    ...
    const checkProduct = () => {
        if (deleteStatus === false) {
            document.getElementById("checkButton").className = "h-screen w-full fixed left-0 top-0 flex justify-center items-center block";
            deleteStatus = !deleteStatus;
        } else {
            document.getElementById("checkButton").className = "h-screen w-full fixed left-0 top-0 flex justify-center items-center hidden";
            deleteStatus = !deleteStatus;
        }
    }
    ...
    const deleteP = (id) => {
        if (id === "") {
            alert("Request not successful. Specify id.");
            return false;
        }
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                alert("Request successful, initializing window refresh");
                location.reload();
        ...
        xhttp.open("POST", "http://localhost/expiry/php/deleteProduct.php", true);
